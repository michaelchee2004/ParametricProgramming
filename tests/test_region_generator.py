...
def test_qp_region_gen(qp_mp_test_data):
    ...
    assert np.allclose(
        ...
        np.array([[-3.16515, -3.7546], [0.07350042, 0.05290033]]),
        rtol=0.0,
        atol=1e-6,
    )
    assert np.allclose(
...
def test_lp_region_gen(lp_mp_test_data):
    ...
    assert np.allclose(
        ...
        np.array([[-0.03278689, 1.0]]),
        rtol=0.0,
        atol=1e-6,
    )
    assert np.allclose(
        mp.regions[1]["added_bound_b"], np.array([213.1147531]), rtol=0.0, atol=1e-6
    )
    assert np.allclose(
    ...
    assert np.allclose(
        mp.regions[1]["firm_bound_b"], np.array([0.0, 6000.0, 0.0]), rtol=0.0, atol=1e-6
    )
