...
class RedundancyChecker:
    ...
    Attributes:
        ...
        relax_tol (float, optional): a float to relax a constraint being checked
            for redundancy. Default provided in config.yml
        zero_tol (float, optional): a float, value higher than this is identified as
            higher than zero. Default provided in config.yml
        x_size (int): int, no of variables in A
        c_size (int): int, no of constraints in A
        redundancy (list of int): a boolean list indicating 1 for each redundant
            constraint
    ...
    def remove_redundancy(self):
        ...
        Returns:
            None
        """
        # for each constraint, delete any old obj, set new obj as Ax of chosen
        # constraint and maximise it.
...
# b = np.array([1.0, -3.58257501, 0.04384359, -0.06334207])
