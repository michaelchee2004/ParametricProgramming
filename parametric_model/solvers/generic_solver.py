...
class GenericSolver:
    ...
    Attributes:
        ...
        lp_activedual_tol (float): duals with values larger than this tolerance is considered 
        ...
        qp_solver_setting (str): qp solver. See admissable solver for pyomo. 
            Default provided in config.yml
        qp_activedual_tol (float): duals with values larger than this tolerance is considered 
            active. See attribute 'active_const'
    ...
    def _create_model(self):        
        ...
        # define solver
        if self.Q is None:
            self.solver = pmo.SolverFactory(self.lp_solver_setting, 
                                            tee=self.tee,
                                            executable=self.lp_solver_path)
        else:
            self.solver = pmo.SolverFactory(self.qp_solver_setting,
                                            tee=self.tee,
                                            executable=self.qp_solver_path)
