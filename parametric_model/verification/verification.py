...
from parametric_model.processing.inputs import load_mp, get_zeros_rows_index
...
np.random.seed(0)
random_inputs = np.random.uniform(low=0., high=1., size=(
    config.app_config.verif_test_size, mp.x_size))
...
for row in random_inputs:
    ...
    opt.solve()
    results_match = np.allclose(mp_soln, opt.soln, rtol=.0, atol=1e-6)
    print(results_match)
